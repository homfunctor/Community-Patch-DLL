/*	-------------------------------------------------------------------------------------------------------
   ...
   Developed by Firaxis Games. Sid Meier's Civilization V, Civ, Civilization, 2K
   Games, Firaxis Games, Take-Two Interactive Software and their respective
   logos are all trademarks of Take-Two interactive Software, Inc. All other
   marks and trademarks are the property of their respective owners. All rights
   reserved.
        -------------------------------------------------------------------------------------------------------
 */
...
bool CvCitySiteEvaluator::CanFoundCity(
    const CvPlot *pPlot, const CvPlayer *pPlayer,
    bool bIgnoreDistanceToExistingCities) const {
  ASSERT(pPlot);
  if (!pPlot)
    return false;
  ...
  if (GC.getGame().isFinalInitialized()) {
    if (GC.getGame().isOption(GAMEOPTION_ONE_CITY_CHALLENGE) && pPlayer &&
        pPlayer->isHuman(ISHUMAN_MECHANICS)) {
  ...
  if (pPlayer) {
    ...
    if (!pPlayer->isHuman(ISHUMAN_AI_DIPLOMACY) && pPlayer->isMajorCiv()) {
      vector<PlayerTypes> vNoSettlePlayers =
          pPlayer->GetDiplomacyAI()->GetPlayersWithNoSettlePolicy();
      for (size_t i = 0; i < vNoSettlePlayers.size(); i++) {
        int iDistanceToOtherPlayer =
            GET_PLAYER(vNoSettlePlayers[i]).GetCityDistanceInPlots(pPlot);
        if (iDistanceToOtherPlayer <= GET_PLAYER(vNoSettlePlayers[i])
                                          .GetDiplomacyAI()
...
/// Setup flavor multipliers - call once per player before PlotFoundValue() or
/// PlotFertilityValue()
void CvSiteEvaluatorForSettler::ComputeFlavorMultipliers(
  ...
  for (int iFlavorLoop = 0; iFlavorLoop < GC.getNumFlavorTypes();
       iFlavorLoop++) {
    ...
    if (strFlavor == "FLAVOR_GROWTH" || strFlavor == "FLAVOR_EXPANSION") {
      m_iFlavorMultiplier[YIELD_FOOD] +=
          pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavor);
      if (pkCitySpecializationEntry) {
        m_iFlavorMultiplier[YIELD_FOOD] +=
            pkCitySpecializationEntry->GetFlavorValue(eFlavor);
      }
    } else if (strFlavor == "FLAVOR_GOLD" ||
               strFlavor == "FLAVOR_TILE_IMPROVEMENT") {
      m_iFlavorMultiplier[YIELD_GOLD] +=
          pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavor);
      if (pkCitySpecializationEntry) {
        m_iFlavorMultiplier[YIELD_GOLD] +=
            pkCitySpecializationEntry->GetFlavorValue(eFlavor);
      ...
      m_iFlavorMultiplier[YIELD_PRODUCTION] +=
          pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavor);
      if (pkCitySpecializationEntry) {
        m_iFlavorMultiplier[YIELD_PRODUCTION] +=
            pkCitySpecializationEntry->GetFlavorValue(eFlavor);
      ...
      m_iFlavorMultiplier[YIELD_SCIENCE] +=
          pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavor) *
          2;
      if (pkCitySpecializationEntry) {
        m_iFlavorMultiplier[YIELD_SCIENCE] +=
            pkCitySpecializationEntry->GetFlavorValue(eFlavor) * 2;
      }
    } else if (strFlavor == "FLAVOR_HAPPINESS") {
      // Doubled since only one flavor related to Happiness
      m_iFlavorMultiplier[SITE_EVALUATION_HAPPINESS] +=
          pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavor) *
          2;
      if (pkCitySpecializationEntry) {
        m_iFlavorMultiplier[SITE_EVALUATION_HAPPINESS] +=
      ...
      m_iFlavorMultiplier[YIELD_FAITH] +=
          pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavor) *
          2;
      if (pkCitySpecializationEntry) {
        m_iFlavorMultiplier[YIELD_FAITH] +=
  ...
  // Set tradable resources and strategic value to times 10 (so multiplying this
  // by the number of map gives a number from 1 to 100)
  m_iFlavorMultiplier[SITE_EVALUATION_RESOURCES] = 10;
  m_iFlavorMultiplier[SITE_EVALUATION_STRATEGIC] = 10;

  // Important: To make sure the resulting scores are comparable, we need to L1
  // normalize this vector (to 100)
  float fSum = 0;
  for (int i = 0; i < NUM_SITE_EVALUATION_FACTORS; i++)
...
/// Retrieve the relative value of this plot (including plots that would be in
/// city radius)
int CvSiteEvaluatorForSettler::PlotFoundValue(
    CvPlot *pPlot, const CvPlayer *pPlayer, const std::vector<int> &ignorePlots,
    bool bCoastOnly, CvString *pDebug) {
  ASSERT(pPlot);
  if (!pPlot)
    return -1;
  ...
  ResourceTypes ePlotResource = pPlot->getResourceType(
      pPlayer->isHuman(ISHUMAN_AI_UNITS) ? eTeam : NO_TEAM);
  ...
  int iBorderlandRange =
      /*5*/ GD_INT_GET(AI_DIPLO_PLOT_RANGE_FROM_CITY_HOME_FRONT);
  ...
  for (int iI = 0; iI < RING_PLOTS[iRange]; iI++) {
    ...
    if (plotDistance(*pLoopPlot, *pPlot) >
        1) // but only if we can't instantly claim them
      if (static_cast<int>(ignorePlots.size()) == GC.getMap().numPlots() &&
          ignorePlots[pLoopPlot->GetPlotIndex()] > 0)
        continue;
    ...
    int iExistingCityDistance =
        GC.getGame().GetClosestCityDistanceInPlots(pLoopPlot);
    int iExistingFriendlyCityDistance =
        pPlayer ? pPlayer->GetCityDistanceInPlots(pLoopPlot) : 5;

    // count the tile only if the city will be able to work it (doesn't need to
    // be passable)
    if (iExistingCityDistance < 2 || pLoopPlot->isBeingWorked())
      iRingModifier = 0;
    else if (iExistingCityDistance == 2)
      // this plot will likely be contested between the two cities, reduce its
      // value
      iRingModifier = 1;

    int iPlotValue = iDefaultPlotValue;
    if (iRingModifier > 0) {
      int iFoodValue = ComputeFoodValue(pLoopPlot, pPlayer) *
                       /*12*/ GD_INT_GET(SETTLER_FOOD_MULTIPLIER);
      int iProductionValue = ComputeProductionValue(pLoopPlot, pPlayer) *
                             /*8*/ GD_INT_GET(SETTLER_PRODUCTION_MULTIPLIER);
      int iGoldValue = ComputeGoldValue(pLoopPlot, pPlayer) *
                       /*8*/ GD_INT_GET(SETTLER_GOLD_MULTIPLIER);
      int iScienceValue = ComputeScienceValue(pLoopPlot, pPlayer) *
                          /*5*/ GD_INT_GET(SETTLER_SCIENCE_MULTIPLIER);
      int iFaithValue = ComputeFaithValue(pLoopPlot, pPlayer) *
                        /*4*/ GD_INT_GET(SETTLER_FAITH_MULTIPLIER);
      // this is about strategic placement, not resources
      int iStrategicValue = ComputeStrategicValue(pLoopPlot, iDistance) *
                            /*3*/ GD_INT_GET(SETTLER_STRATEGIC_MULTIPLIER);

      int iTradeValue = 0;
      int iHappinessValue = 0;
      if (pLoopPlot->getOwner() ==
          NO_PLAYER) // there is no benefit if we already own these tiles
      {
        iHappinessValue = ComputeHappinessValue(pLoopPlot, pPlayer) *
                          /*8*/ GD_INT_GET(SETTLER_HAPPINESS_MULTIPLIER);
        iTradeValue = ComputeTradeableResourceValue(pLoopPlot, pPlayer) *
                      /*3*/ GD_INT_GET(SETTLER_RESOURCE_MULTIPLIER);
      ...
      // it's a bit awkward, happiness and trade value should have a higher
      // modifier in ring 0 ...
      iPlotValue += iRingModifier * (iFoodValue + iProductionValue +
                                     iGoldValue + iScienceValue + iFaithValue +
                                     iTradeValue + iHappinessValue) +
                    iStrategicValue;
    ...
    if (iDistance == 0)
      if (pDebug)
        vQualifiersPositive.push_back(
            CvString::format("raw plot value %d", iPlotValue).c_str());
    ...
    if (pLoopPlot->isMountain() && pPlayer &&
        pPlayer->GetPlayerTraits()->IsMountainPass()) {
  ...
  // take into account only the best 80% of the plots - in the near term the
  // city will not work all plots anyways
  ...
  if (nHammerPlots < 4)
    return 0;
  if (iResourceLuxuryCount < 2 && iResourceStrategicCount < 2 &&
      iResourceBonusCount < 2 && nGoodPlots < 2)
    return 0;
  ...
  if (pPlayer) {
    ...
    } else if (pPlayer->GetPlayerTraits()->GetNaturalWonderYieldModifier() >
               0) {
    ...
    static ImprovementTypes eBrazilImprovement =
        (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_BRAZILWOOD_CAMP",
                                                  true);
    ...
    static ImprovementTypes eMoroccoImprovement =
        (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_KASBAH", true);
    if (eMoroccoImprovement != NO_IMPROVEMENT) {
      CvImprovementEntry *pkEntry = GC.getImprovementInfo(eMoroccoImprovement);
      if (pkEntry != NULL && pkEntry->IsSpecificCivRequired() &&
          !pkEntry->IsAdjacentCity()) {
        CivilizationTypes eCiv = pkEntry->GetRequiredCivilization();
        if (eCiv == pPlayer->getCivilizationType()) {
    ...
    static ImprovementTypes eFranceImprovement =
        (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_CHATEAU", true);
    if (eFranceImprovement != NO_IMPROVEMENT) {
      CvImprovementEntry *pkEntry = GC.getImprovementInfo(eFranceImprovement);
      if (pkEntry != NULL && pkEntry->IsSpecificCivRequired() &&
          pkEntry->IsAdjacentLuxury()) {
        CivilizationTypes eCiv = pkEntry->GetRequiredCivilization();
        if (eCiv == pPlayer->getCivilizationType()) {
    ...
    static ImprovementTypes ePolderImprovement =
        (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_POLDER", true);
    ...
    static ImprovementTypes eIncaImprovement =
        (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_TERRACE_FARM",
                                                  true);
    ...
    static ImprovementTypes eInuitImprovement =
        (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_INUKSUK", true);
  ...
  if (pPlot->IsNaturalWonder()) {
    iValueModifier +=
        (iTotalPlotValue * /*-10*/ GD_INT_GET(BUILD_ON_RESOURCE_PERCENT)) / 100;
    if (pDebug)
  ...
  // food has a higher weight than production anyway, so set a high threshold
  // here
  ...
  if (pPlot->isRiver()) {
    iValueModifier +=
        (iTotalPlotValue * /*35*/ GD_INT_GET(BUILD_ON_RIVER_PERCENT)) / 100;
  ...
  if (pPlot->isCoastalLand(/*10*/ GD_INT_GET(MIN_WATER_SIZE_FOR_OCEAN))) {
    if (nWaterPlots <= 3) {
      iValueModifier += (iTotalPlotValue *
                         /*40*/ GD_INT_GET(SETTLER_BUILD_ON_COAST_PERCENT)) /
                        100;
      if (pDebug)
    ...
    if (pPlayer) {
      int iNavalFlavor =
          pPlayer->GetGrandStrategyAI()->GetPersonalityAndGrandStrategy(
              (FlavorTypes)m_iNavalIndex);
  ...
  if (pPlot->IsChokePoint()) {
    iStratModifier +=
        (iTotalPlotValue * /*10*/ GD_INT_GET(CHOKEPOINT_STRATEGIC_VALUE)) / 100;
    if (pDebug)
  ...
  if (pPlayer && pPlayer->getNumCities() > 0) {
    ...
    {
      // if we're still inside our sphere of influence, see if we can make a
      // strategic claim to territory
      DirectionTypes eDirectionFromCapital =
          directionXY(pPlayer->getCapitalCity()->plot(), pPlot);
      ...
      if (pOutwardPlot) {
        CvCity *pClosestCity2 =
            GC.getGame().GetClosestCityByPathLength(pOutwardPlot, true);
    ...
    if (eNeighbor != NO_PLAYER) {
      int strengthModifiers[NUM_STRENGTH_VALUES] = {100, 100,  50,  0,
                                                    -50, -100, -100};
      int iModifierPercent =
          strengthModifiers[pPlayer->GetDiplomacyAI()
                                ->GetMilitaryStrengthComparedToUs(eNeighbor)];
      ...
      if (iEnemyDistance < iThreshold) {
        ...
        if (pPlayer->GetDiplomacyAI()->GetCivApproach(eNeighbor) <=
                CIV_APPROACH_GUARDED ||
            pPlayer->GetDiplomacyAI()->GetCivOpinion(eNeighbor) <=
      ...
      iModifierPercent +=
          BoldnessModifiers[pPlayer->GetDiplomacyAI()->GetBoldness() - 1];

      // finally
      int iAdjustedEffect =
          (/*30*/ GD_INT_GET(BALANCE_EMPIRE_BORDERLAND_STRATEGIC_VALUE) *
           iModifierPercent) /
          100;
    ...
    if (iOwnCityDistance <= iMinDistance) {
      ...
      iValueModifier -= (20 * iTotalPlotValue) / 100;
      if (pDebug)
        vQualifiersNegative.push_back(
            "(V) too close to existing friendly city");
    }
    ...
    if (iCityDistance >= iSweetMin && iOwnCityDistance <= iSweetMax) {
      iValueModifier +=
          (iTotalPlotValue * 20) / 100; // make this a small bonus, there is a
                                        // separate distance check anyway
      if (pDebug)
        vQualifiersPositive.push_back(
            "(V) optimal distance to existing cities");
    }
  }

  // logging logging logging
  if (pDebug) {
    pDebug->Format("%d,%d,%d,%d", iTotalPlotValue, iValueModifier,
                   iStratModifier, iCivModifier);
  ...
  return max(0,
             iTotalPlotValue + iValueModifier + iStratModifier + iCivModifier);
}
...
int CvCitySiteEvaluator::PlotFertilityValue(CvPlot *pPlot,
                                            ...
                                            bool bIncludeCoast) {
  int rtnValue = 0;
  if ((!pPlot->isWater() && pPlot->isValidMovePlot(NO_PLAYER)) ||
      (bIncludeCoast && pPlot->isShallowWater())) {
...
int CvCitySiteEvaluator::ComputeFoodValue(CvPlot *pPlot,
                                          const CvPlayer *pPlayer) {
  TeamTypes eTeam = pPlayer ? pPlayer->getTeam() : NO_TEAM;
  // From tile yield
  int rtnValue = pPlot->calculateNatureYield(
      YIELD_FOOD, pPlayer ? pPlayer->GetID() : NO_PLAYER,
      pPlot->getFeatureType(), pPlot->getResourceType(eTeam),
      pPlot->getImprovementType(), NULL);

  // assume a farm or similar on suitable terrain ... should be build sooner or
  // later. value averages out with other improvements
  if (((pPlot->getTerrainType() == TERRAIN_GRASS ||
        pPlot->getTerrainType() == TERRAIN_PLAINS) &&
  ...
  if (eResource != NO_RESOURCE) {
    // can we build an improvement on this resource? assume we will do it
    // (natural yield is already considered)

    CvImprovementEntry *pImprovement =
        GC.GetGameImprovements()->GetImprovementForResource(eResource);
    if (pImprovement) {
      rtnValue +=
          pImprovement->GetImprovementResourceYield(eResource, YIELD_FOOD);
    }
  }
...
int CvCitySiteEvaluator::ComputeProductionValue(CvPlot *pPlot,
                                                const CvPlayer *pPlayer) {
  TeamTypes eTeam = pPlayer ? pPlayer->getTeam() : NO_TEAM;
  int rtnValue = pPlot->calculateNatureYield(
      YIELD_PRODUCTION, pPlayer ? pPlayer->GetID() : NO_PLAYER,
      pPlot->getFeatureType(), pPlot->getResourceType(eTeam),
      pPlot->getImprovementType(), NULL);
  // assume a mine or similar in friendly climate. don't run off into the snow
  if (pPlot->isHills() &&
      (pPlot->getTerrainType() == TERRAIN_GRASS ||
       pPlot->getTerrainType() == TERRAIN_PLAINS ||
       pPlot->getTerrainType() == TERRAIN_TUNDRA) &&
      pPlot->getFeatureType() == NO_FEATURE)
    rtnValue += 1;
  ...
  if (eResource != NO_RESOURCE) {
    ...
    CvImprovementEntry *pImprovement =
        GC.GetGameImprovements()->GetImprovementForResource(eResource);
    if (pImprovement) {
      rtnValue += pImprovement->GetImprovementResourceYield(eResource,
                                                            YIELD_PRODUCTION);
    }
  }
...
int CvCitySiteEvaluator::ComputeGoldValue(CvPlot *pPlot,
                                          const CvPlayer *pPlayer) {
  TeamTypes eTeam = pPlayer ? pPlayer->getTeam() : NO_TEAM;
  int rtnValue = pPlot->calculateNatureYield(
      YIELD_GOLD, pPlayer ? pPlayer->GetID() : NO_PLAYER,
      pPlot->getFeatureType(), pPlot->getResourceType(eTeam),
      pPlot->getImprovementType(), NULL);
  ...
  if (eResource != NO_RESOURCE) {
    ...
    if (pImprovement) {
      rtnValue +=
          pImprovement->GetImprovementResourceYield(eResource, YIELD_GOLD);
    }
  }
...
int CvCitySiteEvaluator::ComputeScienceValue(CvPlot *pPlot,
                                             const CvPlayer *pPlayer) {
  TeamTypes eTeam = pPlayer ? pPlayer->getTeam() : NO_TEAM;
  int rtnValue = pPlot->calculateNatureYield(
      YIELD_SCIENCE, pPlayer ? pPlayer->GetID() : NO_PLAYER,
      pPlot->getFeatureType(), pPlot->getResourceType(eTeam),
      pPlot->getImprovementType(), NULL);
  ...
  if (eResource != NO_RESOURCE) {
    ...
    if (pImprovement) {
      rtnValue +=
          pImprovement->GetImprovementResourceYield(eResource, YIELD_SCIENCE);
    }
  }
...
int CvCitySiteEvaluator::ComputeFaithValue(CvPlot *pPlot,
                                           const CvPlayer *pPlayer) {
  TeamTypes eTeam = pPlayer ? pPlayer->getTeam() : NO_TEAM;
  int rtnValue = pPlot->calculateNatureYield(
      YIELD_FAITH, pPlayer ? pPlayer->GetID() : NO_PLAYER,
      pPlot->getFeatureType(), pPlot->getResourceType(eTeam),
      pPlot->getImprovementType(), NULL);
  ...
  if (eResource != NO_RESOURCE) {
    ...
    if (pImprovement) {
      rtnValue +=
          pImprovement->GetImprovementResourceYield(eResource, YIELD_FAITH);
    }
  }
...
int CvCitySiteEvaluator::ComputeHappinessValue(CvPlot *pPlot,
                                               const CvPlayer *pPlayer) {
  int rtnValue = 0;
...
int CvCitySiteEvaluator::ComputeTradeableResourceValue(
    CvPlot *pPlot, const CvPlayer *pPlayer) {
  int rtnValue = 0;

  // If we already own this Tile then we already have access to the Strategic
  // Resource
  ...
  if (eResource != NO_RESOURCE) {
    ResourceUsageTypes eResourceUsage =
        GC.getResourceInfo(eResource)->getResourceUsage();

    // Multiply number of tradeable resources by flavor value
    if (eResourceUsage == RESOURCEUSAGE_LUXURY ||
        eResourceUsage == RESOURCEUSAGE_STRATEGIC) {
      rtnValue += pPlot->getNumResource() *
                  m_iFlavorMultiplier[SITE_EVALUATION_RESOURCES];
      ...
      if (pPlayer) {
        ...
        if (MOD_BALANCE_RESOURCE_MONOPOLIES &&
            (GC.getMap().getNumResources(eResource) > 0)) {
          // Will this get us closer to a monopoly?
          if ((((pPlot->getNumResource() +
                 pPlayer->getNumResourceTotal(eResource, false) +
                 pPlayer->getResourceExport(eResource)) *
                100) /
               GC.getMap().getNumResources(eResource)) >= 30) {
            rtnValue *= 2;
          } else if ((((pPlot->getNumResource() +
                        pPlayer->getNumResourceTotal(eResource, false)) *
                       100) /
                      GC.getMap().getNumResources(eResource)) >= 50) {
            rtnValue *= 10;
          }
        }
      }
...
int CvCitySiteEvaluator::ComputeStrategicValue(CvPlot *pPlot,
                                               int iPlotsFromCity) {
  int rtnValue = 0;
  if (!pPlot)
    return rtnValue;
  ...
  if (pPlot->isHills()) {
    ...
    if (iPlotsFromCity == 1)
      ...
      rtnValue +=
          /*24*/ GD_INT_GET(HILL_STRATEGIC_VALUE) * 12; // good for defense
  }

  // Some Features are less attractive to settle in, (e.g. Jungles, since it
  // takes a while before you can clear them and they slow down movement)
  if (pPlot->getFeatureType() != NO_FEATURE) {
    int iWeight =
        GC.getFeatureInfo(pPlot->getFeatureType())->getStartingLocationWeight();
...
// *****
// ***** This method gets called pre-game if using a WB map (.civ5map), in which
// case pPlayer is NULL
// *****
/// Value of this site for a civ starting location
int CvCitySiteEvaluator::PlotFoundValue(CvPlot *pPlot, const CvPlayer *,
  ...
  // We have our own special method of scoring, so don't call the base class for
  // that (like settler version does)
  for (int iI = 0; iI < RING3_PLOTS; iI++) {
    ...
    if (pLoopPlot == NULL) {
      ...
      int iDistance = plotDistance(pPlot->getX(), pPlot->getY(),
                                   pLoopPlot->getX(), pLoopPlot->getY());
      int iRingModifier = m_iRingModifier[iDistance];

      // Skip the city plot itself for now
      if (iDistance != 0) {
        rtnValue += iRingModifier * ComputeFoodValue(pLoopPlot, NULL) *
                    /*6*/ GD_INT_GET(START_AREA_FOOD_MULTIPLIER);
        rtnValue += iRingModifier * ComputeHappinessValue(pLoopPlot, NULL) *
                    /*12*/ GD_INT_GET(START_AREA_HAPPINESS_MULTIPLIER);
        rtnValue += iRingModifier * ComputeProductionValue(pLoopPlot, NULL) *
                    /*8*/ GD_INT_GET(START_AREA_PRODUCTION_MULTIPLIER);
        rtnValue += iRingModifier * ComputeGoldValue(pLoopPlot, NULL) *
                    /*2*/ GD_INT_GET(START_AREA_GOLD_MULTIPLIER);
        rtnValue += iRingModifier * ComputeScienceValue(pLoopPlot, NULL) *
                    /*1*/ GD_INT_GET(START_AREA_SCIENCE_MULTIPLIER);
        rtnValue += iRingModifier * ComputeFaithValue(pLoopPlot, NULL) *
                    /*1*/ GD_INT_GET(START_AREA_FAITH_MULTIPLIER);
        rtnValue += iRingModifier *
                    ComputeTradeableResourceValue(pLoopPlot, NULL) *
                    /*1*/ GD_INT_GET(START_AREA_RESOURCE_MULTIPLIER);
        rtnValue += iRingModifier *
                    ComputeStrategicValue(pLoopPlot, iDistance) *
                    /*1*/ GD_INT_GET(START_AREA_STRATEGIC_MULTIPLIER);
      }
    }
  }
  ...
  if (pPlot->isCoastalLand(-1, false)) {
    rtnValue +=
        rtnValue * /*20*/ GD_INT_GET(START_AREA_BUILD_ON_COAST_PERCENT) / 100;
  }
